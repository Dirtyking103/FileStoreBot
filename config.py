import os

API_ID = int(os.environ.get("API_ID", "22213072"))
API_HASH = os.environ.get("API_HASH", "8ec41c246b5074ab926933fb286fb43f")
BOT_TOKEN = os.environ.get("BOT_TOKEN", "")
DB_CHANNEL_ID = os.environ.get("DB_CHANNEL_ID", "-1002203224103")
IS_PRIVATE = os.environ.get("IS_PRIVATE",True) # any input is ok But True preferable
OWNER_ID = int(os.environ.get("OWNER_ID", "7202892465"))
UPDATE_CHANNEL = os.environ.get('UPDATE_CHANNEL', '-1002171748949')
AUTH_USERS = list(int(i) for i in os.environ.get("AUTH_USERS", "0").split(" ")) if os.environ.get("AUTH_USERS") else []
if OWNER_ID not in AUTH_USERS:
    AUTH_USERS.append(OWNER_ID)
